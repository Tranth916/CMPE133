    ...
    public class GradAppModel : BindableBase
    {
		...
		public bool WriteAllFields(string outputPath = null)
		{
			...
			try
			{
				if( Stamper == null )
				{
					Debug.WriteLine("Stamper is null? ");
					return false;
				}
					...
					switch (n)
					{
						...
						case "1":
							if (isCurrentEnrolled)
								fieldValue = currentEnrolledCourses[0];
							else if (isUnCompletedKey)
								fieldValue = nonSJSUNotCompleted[0];
							break;

						case "2":
							if (isCurrentEnrolled)
								fieldValue = currentEnrolledCourses[1];
							else if (isUnCompletedKey)
								fieldValue = nonSJSUNotCompleted[1];
							break;

						case "3":
							if (isCurrentEnrolled)
								fieldValue = currentEnrolledCourses[2];
							else if (isUnCompletedKey)
								fieldValue = nonSJSUNotCompleted[2];
							break;

						case "4":
							if (isCurrentEnrolled)
								fieldValue = currentEnrolledCourses[3];
							else if (isUnCompletedKey)
								fieldValue = nonSJSUNotCompleted[3];
							break;

						case "5":
							if (isCurrentEnrolled)
								fieldValue = currentEnrolledCourses[4];
							else if (isUnCompletedKey)
								fieldValue = nonSJSUNotCompleted[4];
							break;

						case "6":
							if (isCurrentEnrolled)
								fieldValue = currentEnrolledCourses[5];
							else if (isUnCompletedKey)
								fieldValue = nonSJSUNotCompleted[5];
							break;

						case "7":
							if (isCurrentEnrolled)
								fieldValue = currentEnrolledCourses[6];
							else if (isUnCompletedKey)
								fieldValue = nonSJSUNotCompleted[6];
							break;

						case "8":
							if (isCurrentEnrolled)
								fieldValue = currentEnrolledCourses[7];
							else if (isUnCompletedKey)
								fieldValue = nonSJSUNotCompleted[7];
							break;
